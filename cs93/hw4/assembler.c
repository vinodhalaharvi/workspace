...
void getFiles(int argc, const char * argv[], FILE **rfile, FILE **wfile){
	assert(argc == 3); 
...
void processLine(char * line, FILE **rfile, FILE ** wfile){
	char *tokens[4]; 
	for (int i = 0; i < 4; i++) {
		tokens[i] = ""; 
	}
...
int main(int argc, const char *argv[])
{
	...
	char * line = NULL; 
	FILE * rfile,  * wfile; 
	getFiles(argc, argv, &rfile, &wfile); 
	while(getline(&line, &len, rfile) != EOF){
		processLine(line, &rfile, &wfile); 
		lineno++; 
	}
	return 0;
}
